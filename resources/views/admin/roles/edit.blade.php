...
@section('title')
    User Role Edit | {{ config('app.name', 'Laravel') }}
@endsection
...
@section('content')
    ...
    <section class="content">
        <div class="container-fluid">
            <form action="{{ route('admin.user.role.update', $role->id) }}" method="POST" enctype="multipart/form-data" onsubmit="disableButton(event)">
                @method('PUT')
                @csrf
                <div class="row">
                    <div class="col-md-4">
                        <div class="card card-primary">
                            ...
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="name" value="{{ old('name', $role->name) }}" placeholder="Enter role name">
                                    @error('name')
                                                ...
                                                <td>
                                                    @forelse($menu->permissions as $permission)
                                                        <div class="icheck-primary d-inline mr-3">
                                                            <input type="checkbox" name="permission_ids[]" value="{{ $permission->id }}" id="permission_{{ $permission->id }}" {{ $role->permissions->contains($permission->id) ? 'checked' : '' }}>
